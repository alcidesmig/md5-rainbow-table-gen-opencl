...
/*
Recursive version for bruteforce
usage:
...
void read_from_file(std::vector<data> & vec, size_t start, size_t size_doc) {
    FILE * fp = fopen(INPUT_FILE, "r");
    fseek(fp, start, SEEK_SET);

    char * buffer = (char *) malloc(size_doc);
    if (!buffer) {
        // printf("Please divide the file (or implement on demand reading :)");
        std::cerr << "Error :(" << std::endl;
    }

    fread(buffer, size_doc, 1, fp);

    char * x = strtok(buffer, "\n");
    while (x) {
        data aux;
        sprintf(aux.value, "%s", x);
        aux.size = strlen(aux.value);
        vec.push_back(aux);
        // std::cout << aux.value << aux.size << std::endl;
        x = strtok(NULL, "\n");
    }
    free(buffer);
    fclose(fp);
 ...
 * forked from: https://github.com/Jsdemonsim/Stackoverflow/blob/214effb54020afcfcbe475a3871674de4b306fe2/alphabet/alphabet.c
 */
